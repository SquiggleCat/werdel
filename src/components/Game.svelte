<script lang="ts">
	...
	function submitWord() {
		...
		if (getCubes(game.board.words[game.guesses], false).length < COLS) {
			toaster.pop("Not enough letters");
			board.shake(game.guesses);
		//} else if (words.contains(game.board.words[game.guesses])) {
		} else if (getCubes(game.board.words[game.guesses], false).length > COLS) {
			toaster.pop("Too many letters");
			board.shake(game.guesses);
		} else if (ipaDict.contains(getCubesStr(game.board.words[game.guesses]))) {
			if (game.guesses > 0) {
